...
protected trait AVX51201 extends IntrinsicsBase {
  /**
...
protected trait CGenAVX51201 extends CGenIntrinsics {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case MM512_MASK_EXPANDLOADU_PD(src, k, mem_addr, mem_addrOffset) =>
      emitValDef(sym, s"_mm512_mask_expandloadu_pd(${quote(src)}, ${quote(k)}, ${quote(mem_addr) + (if(mem_addrOffset == Const(0)) "" else " + " + quote(mem_addrOffset))})")
    case MM512_MASKZ_EXPAND_PD(k, a) =>
      emitValDef(sym, s"_mm512_maskz_expand_pd(${quote(k)}, ${quote(a)})")
    case MM512_MASKZ_EXPANDLOADU_PD(k, mem_addr, mem_addrOffset) =>
      emitValDef(sym, s"_mm512_maskz_expandloadu_pd(${quote(k)}, ${quote(mem_addr) + (if(mem_addrOffset == Const(0)) "" else " + " + quote(mem_addrOffset))})")
    case MM512_MASK_EXPAND_PS(src, k, a) =>
      emitValDef(sym, s"_mm512_mask_expand_ps(${quote(src)}, ${quote(k)}, ${quote(a)})")
    case MM512_MASK_EXPANDLOADU_PS(src, k, mem_addr, mem_addrOffset) =>
      emitValDef(sym, s"_mm512_mask_expandloadu_ps(${quote(src)}, ${quote(k)}, ${quote(mem_addr) + (if(mem_addrOffset == Const(0)) "" else " + " + quote(mem_addrOffset))})")
    case MM512_MASKZ_EXPAND_PS(k, a) =>
      emitValDef(sym, s"_mm512_maskz_expand_ps(${quote(k)}, ${quote(a)})")
    case MM512_MASKZ_EXPANDLOADU_PS(k, mem_addr, mem_addrOffset) =>
      emitValDef(sym, s"_mm512_maskz_expandloadu_ps(${quote(k)}, ${quote(mem_addr) + (if(mem_addrOffset == Const(0)) "" else " + " + quote(mem_addrOffset))})")
    case MM512_EXTRACTF32X4_PS(a, imm8) =>
